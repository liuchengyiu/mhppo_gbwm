...
from agent.hppo_mine import HPPO
...
def evaluate_policy(args, env, agent, state_norm):
    ...
    if evaluate_reward / times > 2450:
        pd.DataFrame(single_re).to_csv("hppo_{}.csv".format(evaluate_reward / times))
    return evaluate_reward / times


def main(args, env_name, number, seed):
    log_dir = "./train_log/no_infuse/"
    ...
    while total_steps < args.max_train_steps:
        ...
        while not done:
            ...
            if total_steps % args.batch_size == 0:
                ...
                writer.add_scalar('step_rewards_no_infu_{}'.format(agent.ac_type), evaluate_rewards[-1], global_step=total_steps)
                # Save the rewards
