...
from agent.paddpg import PADDPG
...
def main(args, env_name, number, seed):
    ...
    state_norm = Normalization(shape=env.state_dim) 
    reward_scaling.reset()
    episode_steps = 0
    evaluate_num = 0
    import tqdm
    for episode_steps in range(100000):
        s = env.reset()
        episode_steps += 1
        ...
        while not done:
            ...
            r_ = reward_scaling(r)
            agent.step(s, (all_actions, all_action_parameters), r_, s_, done,
                       optimise=True)
            s = s_
