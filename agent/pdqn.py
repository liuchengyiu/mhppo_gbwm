...
from lib.utils import soft_update_target_network, hard_update_target_network
...
class QActorDualing(nn.Module):

    def __init__(self, state_size, action_size, action_parameter_size, hidden_layers=(100,), action_input_layer=0,
                 output_layer_init_std=None, activation="relu", **kwargs):
        super(QActorDualing, self).__init__()
        self.state_size = state_size
        self.action_size = action_size
        self.action_parameter_size = action_parameter_size
        self.activation = activation
        ...
        self.fc_a = nn.Linear(last_hidden_layer_dim, action_size)
...
class PDQNAgent(Agent):
    ...
    def __init__(self,
        ...
        self.num_actions = action_space_dim
        self.action_parameter_sizes = np.array([2]*action_space_dim)
        self.action_parameter_size = int(self.action_parameter_sizes.sum())
        self.action_parameter_max_numpy = np.array([1,1]*action_space_dim)
        self.action_parameter_min_numpy = np.array([0,0]*action_space_dim)
        ...
        self.replay_memory = Memory(replay_memory_size, (observation_space_dim,), (1+self.action_parameter_size,), next_actions=False)
        self.actor = actor_class(observation_space_dim, self.num_actions, self.action_parameter_size, **actor_kwargs).to(device)
        self.actor_target = actor_class(observation_space_dim, self.num_actions, self.action_parameter_size, **actor_kwargs).to(device)
        hard_update_target_network(self.actor, self.actor_target)
        self.actor_target.eval()

        self.actor_param = actor_param_class(observation_space_dim, self.num_actions, self.action_parameter_size, **actor_param_kwargs).to(device)
        self.actor_param_target = actor_param_class(observation_space_dim, self.num_actions, self.action_parameter_size, **actor_param_kwargs).to(device)
