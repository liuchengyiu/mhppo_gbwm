...
class ActorDis(nn.Module):
    def __init__(self, state_dim, action_dim, param_dim):
        ...
        last_layer_dim = state_dim + param_dim
        ...
        self.param_pass = nn.Linear(param_dim, action_dim)
        xavier_init(self.param_pass)
...
class HPPO:
    def __init__(self, 
        ...
        print("mix_a")
        self.actor_dis = ActorDis(state_dim=state_dim, action_dim=action_dim, param_dim=param_dim).to(self.device)
    ...
    def evalate(self, s):
        ...
        with torch.no_grad():
            ...
            a_prob = self.actor_dis(s,torch.unsqueeze(torch.tensor(p, dtype=torch.float), dim=0).to(self.device)).detach().cpu().numpy().flatten()
            a = np.argmax(a_prob)
        return a, p
    ...
    def update(self, replay_buffer, total_steps):
        ...
        for _ in range(self.k_epoch):
            for index in BatchSampler(SubsetRandomSampler(range(self.batch_size)), self.mini_batch_size, False):
                ...
                #update actor
                dist_now = Categorical(probs=self.actor_dis(s[index], p[index]))
