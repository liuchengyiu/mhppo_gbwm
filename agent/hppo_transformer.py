...
from net.my_transformer import TransformerEncoder
...
class ActorDis(nn.Module):
    def __init__(self, state_dim, action_dim, vocab_size):
        super(ActorDis, self).__init__()
        d_model = state_dim 
        self.encoder = TransformerEncoder(state_dim=state_dim, vocab_size=vocab_size, d_model=d_model, hidden_fc_size=256, block_num=3, dim=32, n_heads=3)
        last_layer_dim = d_model
    ...
    def forward(self, x):
        ...
        for i in range(len(self.layers)-1):
            x = F.tanh(self.layers[i](x))
        action = F.softmax(self.layers[-1](x))
...
class HPPO:
    def __init__(self, 
        ...
        self.actor_dis = ActorDis(state_dim=state_dim, action_dim=action_dim, vocab_size=vocab_size).to(self.device)
        summary(self.actor_dis, input_size = (3, 3,state_dim), device=self.device)
        if ac_type == 'beta':
    ...
    def update(self, replay_buffer, total_steps):
        ...
        for epoch in range(self.k_epoch):
            ...
            for index in BatchSampler(SubsetRandomSampler(range(self.batch_size)), self.mini_batch_size, False):
                ...
                probs = torch.zeros(len(index), self.action_dim).to(self.device)
                for i, ind in enumerate(index):
                    probs[i, :] = self.actor_dis(s[ind-ind % self.t:ind+1])[0]
    ...
    def save(self, path):
        torch.save(self.actor_con.state_dict(), os.path.join(path, "actor_con.pt"))
        torch.save(self.actor_dis.state_dict(), os.path.join(path, "actor_dis.pt"))
        torch.save(self.critic.state_dict(), os.path.join(path, "critic.pt"))
    
    def load(self, path):
        self.actor_con.load_state_dict(torch.load(os.path.join(path, "actor_con.pt")))
        self.actor_dis.load_state_dict(torch.load(os.path.join(path, "actor_dis.pt")))
