...
class ActorCritic_Continuous(nn.Module):
    def __init__(self,
        ...
        # log_std is a trainable tensor to implement exploration and exploitation trade-off
...
class ActorCritic_Hybrid(nn.Module):
    ...
    def get_logprob_entropy(self, state, action_dis, action_con):
        action_probs = self.actor_dis(state)
        dist_dis = Categorical(action_probs)
...
class PPO_Discrete(PPO_Abstract, ABC):
    def __init__(self, state_dim, action_dim, mid_dim, lr_actor_dis, lr_critic, lr_decay_rate, buffer_size, target_kl_dis,
                 gamma, lam, epochs_update, v_iters, eps_clip, max_norm, coeff_entropy, random_seed, device):
        super().__init__(state_dim, action_dim, mid_dim, buffer_size, gamma, lam, epochs_update, v_iters,
                         eps_clip, max_norm, coeff_entropy, random_seed, device)
        ...
        self.optimizer_actor = torch.optim.Adam(self.agent.actor.parameters(), lr=lr_actor_dis)
    ...
    def select_action(self, state):
        ...
        return state_value.cpu().numpy(), action.cpu().numpy(), log_prob.cpu().numpy()
    ...
    def update(self, batch_size):
        ...
        for i in range(self.epochs_update):
            ...
            for data in sampler:
                ...
                # early stopping trick in openai spinning up, please refer to spinning up for a more detailed introduction
                # it's may be a wrong implementation, more likely a filter
...
class PPO_Continuous(PPO_Abstract, ABC):
    def __init__(self, state_dim, action_dim, mid_dim, lr_actor_con, lr_critic, lr_decay_rate, buffer_size, target_kl_con,
                 gamma, lam, epochs_update, v_iters, eps_clip, max_norm, coeff_entropy, random_seed, device,
                 lr_std, init_log_std):
        super().__init__(state_dim, action_dim, mid_dim, buffer_size, gamma, lam, epochs_update,
                         v_iters, eps_clip, max_norm, coeff_entropy, random_seed, device)
        ...
        self.optimizer_actor = torch.optim.Adam([
            {'params': self.agent.actor.parameters(), 'lr': lr_actor_con},
...
class PPO_Hybrid(PPO_Abstract, ABC):
    def __init__(self, state_dim, action_dim, mid_dim, lr_actor_con, lr_actor_dis, lr_critic, lr_decay_rate, buffer_size, target_kl_dis, target_kl_con,
                 gamma, lam, epochs_update, v_iters, eps_clip, max_norm, coeff_entropy, random_seed, device, init_log_std):
        super().__init__(state_dim, action_dim, mid_dim, buffer_size, gamma, lam, epochs_update,
                         v_iters, eps_clip, max_norm, coeff_entropy, random_seed, device)
        ...
        self.optimizer_actor_con = torch.optim.Adam([
            {'params': self.agent.actor_con.parameters(), 'lr': lr_actor_con},
        ])
        self.optimizer_actor_dis = torch.optim.Adam([
            {'params': self.agent.actor_dis.parameters(), 'lr': lr_actor_dis},
        ])
        self.optimizer_critic = torch.optim.Adam(self.agent.critic.parameters(), lr=lr_critic)
    ...
    def select_action(self, state):
        ...
        return state_value.cpu().numpy(), (action_dis.cpu().numpy(), action_con.cpu().numpy()), (log_prob_dis.cpu().numpy(), log_prob_con.cpu().numpy())
